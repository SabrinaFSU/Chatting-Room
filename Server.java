...
//    arrives at a server. For the Transmission Control Protocol and the
//    User Datagram Protocol, a port number is a 16-bit integer that is
//    put in the header appended to a message unit.
    int portNumber = 4119;
    if (args.length < 1) {
      ...
      try {
        ...
        for (i = 0; i < maxClientsCount; i++) {
          if (threads[i] == null) {
            ...
            //Socket clientSocket, clientThread[] threads,Map<String,String> amap,Map<String,Demo> bmap,Map<String,String> aoffMmap,
            // Map<String,Long> atimeOffMap, Map<String,ArrayList<String>> agroupTalk)//!!!!Wrong!!!!!!!!!
            // (offThread=new monitorThread(threads, timeOutMap, maxClientsCount)).start();
            break;
          }
        }
